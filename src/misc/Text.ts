...
export default class Text {
  ...
  constructor(data: any) {
    if (typeof data === 'object' && data !== null && Reflect.has(data, 'runs') && Array.isArray(data.runs)) {
      this.runs = data.runs?.map((run: any) => (run.emoji ? new EmojiRun(run) : new TextRun(run))) ?? []
    ...
    if (style_runs || command_runs || attachment_runs) {
      if (style_runs) {
        for (const style_run of style_runs) {
          if (
            ...
            if (!matching_run) {
              console.warn(TAG, 'Unable to find matching run for style run. Skipping...', {
                style_run,
                input_data: data,
            ...
            console.debug(TAG, "Skipping style run as it is doesn't have any information that we parse.", {
              style_run,
              input_data: data,
            })
          }
      ...
      if (command_runs) {
        for (const command_run of command_runs) {
          if (command_run.onTap) {
            ...
            if (!matching_run) {
              console.warn(TAG, 'Unable to find matching run for command run. Skipping...', {
                command_run,
                input_data: data,
                // For performance reasons, web browser consoles only expand an object, when the user clicks on it,
                // So if we log the original runs object, it might have changed by the time the user looks at it.
            ...
            console.debug(TAG, 'Skipping command run as it is missing the "doTap" property.', {
              command_run,
              input_data: data,
            })
          }
      ...
      if (attachment_runs) {
        for (const attachment_run of attachment_runs) {
          ...
          if (!matching_run) {
            console.warn(TAG, 'Unable to find matching run for attachment run. Skipping...', {
              attachment_run,
              input_data: data,
              // For performance reasons, web browser consoles only expand an object, when the user clicks on it,
              // So if we log the original runs object, it might have changed by the time the user looks at it.
          ...
          if (attachment_run.length === 0) {
            ...
            if (
              attachment_run.element?.type?.imageType?.image &&
              (is_custom_emoji || /^(?:\p{Emoji}|\u200d)+$/u.test(text))
            ) {
              const emoji = {
...
function insertSubRun(
  ...
  properties_to_add: Omit<RawRun, 'text' | 'startIndex'>,
) {
